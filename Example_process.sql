...
/*Most of the constraint names are user-defined whereas in case of the primary key constraints of tables Comment and Person_state_type the name is system-generated.*/
...
ALTER TABLE public.Person_feedback ADD CONSTRAINT fk_person_feedback_person FOREIGN KEY(author) REFERENCES Person(person_id);


/*Column first_name does not have a check constraint to prevent the empty string or strings that consist of only whitespace characters.*/
ALTER TABLE public.Person ADD CONSTRAINT chk_person_given_name CHECK(given_name!~'^[[:space:]]*$');


/*Columns with a timestamp type (check_date, registration_time, comment_registration_time) do not have a check constraint to prevent illogical values that are far back in time (for instance, 1200-12-22) or in the distant future. Such values belong to the type but are inappropriate to use in case of the domain.*/
...
/*The check ensuring that last names cannot contain numbers is too restrictive.*/
...
/*Columns for registering personal names (first_name, last_name) do not take into account a possibility of long names and the existence of mononymous persons.*/
...
/*Column person_state_type_code of table Person_data does not have a default value. The value should be the code of the first state that a person gets according to the state machine model of entity type Person.*/
ALTER TABLE public.Person ALTER COLUMN persons_state_type_code SET DEFAULT 1;
...
/*In table Comment column registration_time has a default value whereas in table Public_person_data it does not (inconsistency).*/
...
/*Column name check_date is not consistent with the datatype leading to the question what should be the precision of data (date or timestamp) in this column.*/
...
/*Column active of Comment is optional leading to the use of three-valued logic.*/
ALTER TABLE public.Person_feedback ALTER COLUMN is_active SET NOT NULL;


/*Comments have a simplistic state machine (active/inactive). The task to register a current state is implemented differently in case of different tables (inconsistency).*/
...
/*Foreign key column persons_state_type_code of Person is not indexed.*/
CREATE INDEX idx_person_persons_state_type_code ON Person (persons_state_type_code);

--There is now another foreign key column (author of Person_feedback) that needs index as well.
CREATE INDEX idx_person_feedback_author ON Person_feedback (author);


/*Foreign key constraint between tables Public_person_data and Person_state_type does not have ON UPDATE CASCADE compensating action. Moreover, having ON DELETE CASCADE compensating action there is reckless.*/
ALTER TABLE Person DROP CONSTRAINT fk_person_person_state_type;
ALTER TABLE Person ADD CONSTRAINT fk_person_person_state_type FOREIGN KEY (persons_state_type_code) REFERENCES Person_state_type (person_state_type_code) ON UPDATE CASCADE;


/*One letter difference of the primary/foreign key column names (person_state_type_code vs. persons_state_type_code) makes writing join queries more difficult and error prone and prevents the use of USING syntax in the join.*/
